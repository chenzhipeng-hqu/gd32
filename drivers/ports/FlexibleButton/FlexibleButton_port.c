...
#define PIN_KEY_WKUP    GET_PIN(A, 0)

#define PIN_KEY_TAMPER  GET_PIN(C, 13)

#define PIN_KEY_USER    GET_PIN(B, 14)
...
static char *enum_btn_id_string[] = {
    ENUM_TO_STR(BUTTON_WKUP),
    ENUM_TO_STR(BUTTON_TAMPER),
    ENUM_TO_STR(BUTTON_USER),
...
static uint8_t common_btn_read(void *arg)
{
    ...
    {
    case BUTTON_WKUP:
        value = rt_pin_read(PIN_KEY_WKUP);
        break;
    case BUTTON_TAMPER:
        value = rt_pin_read(PIN_KEY_TAMPER);
        break;
    case BUTTON_USER:
        value = rt_pin_read(PIN_KEY_USER);
        break;
    default:
        RT_ASSERT(0);
    }
...
static void common_btn_evt_cb(void *arg)
{
    ...
    if ((flex_button_event_read(&user_button[BUTTON_TAMPER]) == FLEX_BTN_PRESS_CLICK) 
        && (flex_button_event_read(&user_button[BUTTON_USER]) == FLEX_BTN_PRESS_CLICK))
    {
        rt_kprintf("[combination]: %s and %s click\n", enum_btn_id_string[BUTTON_TAMPER], enum_btn_id_string[BUTTON_USER]);
    }
}
...
static void user_button_init(void)
{
    ...
    rt_pin_mode(PIN_KEY_WKUP, PIN_MODE_INPUT_PULLUP); /* set KEY pin mode to input */
    rt_pin_mode(PIN_KEY_TAMPER, PIN_MODE_INPUT_PULLUP); /* set KEY pin mode to input */
    rt_pin_mode(PIN_KEY_USER, PIN_MODE_INPUT_PULLUP); /* set KEY pin mode to input */
